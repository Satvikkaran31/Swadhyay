...
export async function enroll(req, res) {
  ...
  try {
    ...
    if (course.price > 0) {
      // Paid courses must go through /api/payment/verify — that route handles
      // HMAC verification and enrollment atomically.
      return res.status(400).json({ error: 'Use the payment flow to enroll in paid courses' });
    }
